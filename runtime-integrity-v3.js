// Runtime coordination for day rollover, stored cards, tabs, and four-step guided progress.
(() => {
  ...
  function syncGuidedDayFromExternalActivity() {
    ...
    if (todayCorrect) {
      Object.assign(day.script, { studyComplete: true, phase: 'today', todayAnswered: true, todaySelected: todayLetter, todayCorrect: true });
      localStorage.setItem(`${STUDIED_PREFIX}${key}`, '1');
    }
    day.done.script = Boolean(day.script.todayCorrect || todayCorrect);
    ...
    if (allDone) {
      ...
      const invalid = !Number.isInteger(step) || step < 0 || step > STEP_KEYS.length;
      if (invalid || step === STEP_KEYS.length || day.done[STEP_KEYS[step]]) day.step = nextIncomplete(day);
    }
